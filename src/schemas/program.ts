import { type z, object, string, number, boolean, date, union, intersection, ZodIssueCode } from 'zod'
...
export const countryProgramSchema = object({
...
export const programSchema = object({
  ...
  programTraining: boolean().nullish().default(false),
...
export const registerProgramSchema = intersection(countryProgramSchema, programSchema)
  .transform(arg => {
...
export const updateProgramSchema = intersection(countryProgramSchema, programSchema).and(IdSchema)
  .transform(arg => {
    const { training, ...data } = arg
    return {
      ...data,
